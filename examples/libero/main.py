...
def eval_libero(args: Args) -> None:
    ...
    for task_id in tqdm.tqdm(range(num_tasks_in_suite)):
        ...
        for episode_idx in tqdm.tqdm(range(args.num_trials_per_task)):
            ...
            if replay_images:
                if done:
                    ...
                    last_failure_images = [np.asarray(x) for x in replay_images]
        ...
        if last_success_images is not None:
            imageio.mimwrite(
                pathlib.Path(args.video_out_path) / f"rollout_{task_segment}_success.mp4",
                last_success_images,
                fps=10,
            )
            logging.info(f"Saved last success video for task: {task_description}")

        if last_failure_images is not None:
            imageio.mimwrite(
                pathlib.Path(args.video_out_path) / f"rollout_{task_segment}_failure.mp4",
                last_failure_images,
                fps=10,
            )
            logging.info(f"Saved last failure video for task: {task_description}")
    ...
    # log json version final result
    if args.log_dir:
        eval_results_json["total_episodes"] = total_episodes
        eval_results_json["total_successes"] = total_successes
        eval_results_json["total_success_rate"] = (
